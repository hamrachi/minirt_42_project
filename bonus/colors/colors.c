...
/*   Updated: 2025/02/23 20:32:44 by elel-bah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../miniRT_bonus.h"

t_point3d	compute_lighting(t_world *scene, \
	t_inter_data intersection, t_point3d ambient_color)
{
	...
	while (current_light)
	{
		...
		{
			light_direction = sub_vec(current_light->position, \
				intersection.hit_point);
			diffuse_factor = dot_product(scale_to_one(light_direction), \
				intersection.normal);
			if (diffuse_factor > 0)
				final_color = blend_colors(final_color, \
					diffuse(intersection, current_light, diffuse_factor));
			final_color = blend_colors(final_color, \
				specular(scene, intersection, current_light));
		}
